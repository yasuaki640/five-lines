...
テストフレームワークは無い。本リポジトリは **書籍学習用の試行リファクタリング** であり、手動プレイによる動作確認はコストに見合わないためスキップする。Claude は変更後に「ブラウザで動作確認してください」と促さない。`tsc` が通ることだけ確認すれば十分。スクリーンショット (`game.png`, `goal.png`) は参考情報として残すが、毎回の照合は行わない。
